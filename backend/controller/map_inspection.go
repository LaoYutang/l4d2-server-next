...
func GetMapScriptOverrides(c *gin.Context) {
	var request struct {
		Map string `json:"map"`
	}
	if err := c.ShouldBindJSON(&request); err != nil {
	...
	mutex.RLock()
	scripts, err := logic.GetMapScriptOverrideContents(mapName)
	mutex.RUnlock()
	if err != nil {
		switch {
		case errors.Is(err, logic.ErrMapInspectionStale):
		...
		case errors.Is(err, logic.ErrMapInspectionNotFound),
			errors.Is(err, logic.ErrMapRecordNotFound),
			errors.Is(err, logic.ErrNoScriptOverrides),
			os.IsNotExist(err):
			FailWithError(c, http.StatusNotFound, "%v", err)
		default:
			FailWithError(c, http.StatusInternalServerError, "读取覆盖脚本失败: %v", err)
		}
		return
	}
