...
func TestLogOpRecordsFinalRequestResult(t *testing.T) {
	...
	records := make([]model.AuditLog, 0, 3)
	...
	for _, path := range []string{"/success", "/failure", "/panic"} {
	...
	if records[2].Success || records[2].Detail != "panic detail" {
		t.Fatalf("unexpected panic record: %+v", records[2])
	}
}

func TestListAuditLogsRejectsGuest(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body, _ := json.Marshal(AuditListRequest{Page: 1, PageSize: 20})
