...
管理员可以在“访问控制”页面配置面板黑白名单和可信代理，保存后立即生效，不需要重启管理器。规则支持地区关键字、单 IP 和 CIDR；地区规则依赖后端随包提供的 `ip2region_*.xdb` 数据文件。

旧的 `REGION_WHITE_LIST` 环境变量已取消且不会自动迁移。升级后需要在页面中重新配置面板规则。首次没有页面配置时，面板黑白名单默认关闭，可信代理默认为 `127.0.0.0/8` 和 `::1/128`。

可信代理决定管理器是否接受 `X-Forwarded-For` 和 `X-Real-IP`。只应填写自己实际控制的反向代理地址；代理未正确配置时，管理器会把代理地址当作客户端地址。详细说明见[面板访问控制](/features/access-control)。
