...
“访问控制”仅管理员可见，用于限制 Web 管理面板访问，不会修改 L4D2 玩家封禁或 SourceMod 配置。访问判断在登录页、静态资源和全部 API 之前执行。

访问控制统一在管理面板中配置，不使用环境变量。配置保存后立即生效，无需重启管理器。
...
“面板黑白名单”和“可信代理”两个 Tab 独立保存。多名管理员同时修改配置时，后保存的一方可能收到配置冲突提示；刷新配置后再重新编辑即可。
...
| `409` | 草稿会锁定当前管理员，或配置已被其他管理员同时修改。 |
| `503` | 地区规则需要 GeoIP 但服务不可用，或配置文件损坏后处于恢复模式。 |
